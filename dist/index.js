...
var HostedForm = function (_a) {
    ...
    var _h = React__namespace.useState(true), loading = _h[0], setLoading = _h[1];
    var scriptUrl = environment === 'PRODUCTION'
        ? 'https://js.authorize.net/v3/AcceptUI.js'
        : 'https://jstest.authorize.net/v3/AcceptUI.js';
    var _j = useScript(scriptUrl), scriptLoaded = _j[0], scriptError = _j[1];
    React__namespace.useEffect(function () {
        if (scriptLoaded || scriptError) {
            setLoading(false);
        }
    }, [scriptLoaded, scriptError]);
    var _k = React__namespace.useState(null), errors = _k[0], setErrors = _k[1];
    var responseHandler = React__namespace.useCallback(function (response) {
    ...
    return (jsxRuntime.jsxs("div", __assign({ style: containerStyle, className: containerClassName ? containerClassName : undefined }, { children: [jsxRuntime.jsx("button", __assign({ type: "button", style: buttonStyle, className: buttonClassName ? "AcceptUI ".concat(buttonClassName) : 'AcceptUI', "data-billingaddressoptions": JSON.stringify(billingAddressOptions), "data-apiloginid": authData.apiLoginID, "data-clientkey": authData.clientKey, "data-acceptuiformbtntxt": formButtonText, "data-acceptuiformheadertxt": formHeaderText, "data-paymentoptions": JSON.stringify(paymentOptions), "data-responsehandler": "responseHandler", disabled: scriptError || loading || disabled }, { children: buttonText })), errors && (jsxRuntime.jsx("div", __assign({ style: errorTextStyle, className: errorTextClassName !== null && errorTextClassName !== void 0 ? errorTextClassName : '' }, { children: typeof errors === 'string'
                    ? errors
                    : errors.map(function (error, index) { return (jsxRuntime.jsxs("div", { children: ["`Error ($", error.code, "): $", error.text, ")`"] }, "error-".concat(index))); }) })))] })));
};
...
var RedirectIntegration = function (_a) {
    ...
    return (jsxRuntime.jsxs("form", __assign({ method: "post", action: postUrl }, { children: [jsxRuntime.jsx("input", { type: "hidden", name: "token", value: formToken }), jsxRuntime.jsx("button", __assign({ className: className, style: className ? {} : style }, { children: children }))] })));
};
...
var IFrameIntegrationContext = React.createContext(null);
var useIFrameIntegrationContext = function () {
    var context = React.useContext(IFrameIntegrationContext);
    if (!context) {
...
var IFrameIntegration = function (_a) {
    ...
    var _b = React.useState(false), popupIsShown = _b[0], setPopupIsShown = _b[1];
    var popupFormRef = React.useRef(null);
    ...
    return (jsxRuntime.jsx(IFrameIntegrationContext.Provider, __assign({ value: contextValue }, { children: children })));
};
var IFrameIntegrationButton = function (_a) {
    ...
    return (jsxRuntime.jsxs("form", __assign({ method: "post", action: postUrl, id: "formAuthorizeNetPopup", name: "formAuthorizeNetPopup", target: "iframeAuthorizeNet", onSubmit: handleShowPopup }, { children: [jsxRuntime.jsx("input", { type: "hidden", id: "popupToken", name: "token", value: formToken }), jsxRuntime.jsx("button", __assign({ disabled: popupIsShown, className: className, style: style }, { children: children }))] })));
};
var IFrameContainer = function (_a) {
    ...
    var _c = React.useState(0), offsetWidth = _c[0], setOffsetWidth = _c[1];
    var _d = React.useState(0), offsetHeight = _d[0], setOffsetHeight = _d[1];
    // React.useEffect(() => {
    //   if (popupIsShown) {
    //     debugger;
    //   }
    ...
    return (jsxRuntime.jsx("div", __assign({ className: className, ref: popupRef, style: className
...
var IFrameBackdrop = function (_a) {
    ...
    return (jsxRuntime.jsx("div", { className: className, style: className
            ? { display: popupIsShown ? '' : 'none' }
...
var AcceptHosted = function (props) {
    ...
    if (props.integration === 'redirect') {
        return (jsxRuntime.jsx(RedirectIntegration, __assign({ formToken: props.formToken, postUrl: postUrl }, { children: props.children })));
    }
    if (props.integration === 'iframe') {
        return (jsxRuntime.jsx(IFrameIntegration, __assign({ formToken: props.formToken, postUrl: postUrl, onTransactionResponse: props.onTransactionResponse, onCancel: props.onCancel, onResize: props.onResize, onSuccessfulSave: props.onSuccessfulSave }, { children: props.children })));
    }
    return null;
};
...
exports.AcceptHosted = AcceptHosted;
exports.HostedForm = HostedForm;
exports.useAcceptJs = useAcceptJs;
